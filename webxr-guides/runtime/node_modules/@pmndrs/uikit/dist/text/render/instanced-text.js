...
import { getGlyphLayoutHeight, getGlyphOffsetX, getGlyphOffsetY, getOffsetToNextGlyph, getOffsetToNextLine, toAbsoluteNumber, } from '../utils.js';
import { buildGlyphLayout, computedCustomLayouting } from '../layout.js';
export const additionalTextDefaults = {
    verticalAlign: 'middle',
};
export function createInstancedText(text, parentClippingRect, selectionRange, selectionTransformations, caretTransformation, instancedTextRef) {
    ...
    abortableEffect(() => text.node.addLayoutChangeListener(() => {
        const layoutProperties = layoutPropertiesRef.current;
        const { size: { value: size }, paddingInset: { value: paddingInset }, borderInset: { value: borderInset }, } = text;
    ...
    abortableEffect(() => {
        ...
        const instancedText = new InstancedText(text.root.value.glyphGroupManager.getGroup(text.orderInfo.value, text.properties.value.depthTest, text.properties.value.depthWrite ?? false, text.properties.value.renderOrder, font), text.properties, layoutSignal, text.globalTextMatrix, text.isVisible, parentClippingRect, selectionRange, selectionTransformations, caretTransformation);
...
export class InstancedText {
    ...
    unsubscribeShowList = [];
    constructor(group, properties, layoutSignal, matrix, isVisible, parentClippingRect, selectionRange, selectionTransformations, caretTransformation) {
        this.group = group;
        ...
        this.unsubscribeInitialList = [
            ...
            effect(() => this.updateSelectionBoxes(this.lastLayout, selectionRange?.value, properties.peek().verticalAlign, properties.peek().textAlign)),
        ];
    }
    ...
    updateSelectionBoxes(layout, range, verticalAlign, textAlign) {
        ...
        const newSelectionTransformations = [
        ...
        for (let i = start.lineIndex + 1; i < end.lineIndex; i++) {
            newSelectionTransformations.push(this.computeSelectionTransformation(i, undefined, undefined, layout, verticalAlign, whitespaceWidth));
        }
    ...
    computeSelectionTransformation(lineIndex, startX, endX, layout, verticalAlign, whitespaceWidth) {
    ...
    show() {
        ...
        this.unsubscribeShowList.push(effect(() => {
            ...
            for (let lineIndex = 0; lineIndex < linesLength; lineIndex++) {
                ...
                let offsetPerWhitespace = textAlign === 'justify' ? (availableWidth - nonWhitespaceWidth) / whitespacesBetween : 0;
            ...
            this.updateSelectionBoxes(layout, this.selectionRange?.peek(), this.properties.value.verticalAlign, this.properties.value.textAlign);
        }));
    }
    hide() {
